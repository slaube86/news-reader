...
const messages: Record<string, Record<string, string>> = {
  de: {
    ...
    "feeds.allUpdatedAggregator":
      "Alle Quellen wurden aktualisiert (Aggregator)",
    "feeds.loadError":
      "Konnte nicht laden: {detail}. CORS-Proxy kann zeitweise überlastet sein.",
  ...
  en: {
    ...
    "feeds.loadError":
      "Could not load: {detail}. CORS proxy may be temporarily overloaded.",
...
/**
 * Determines the translation TARGET language from the browser/OS locale.
 * Supports three languages: German (de), English (en), Farsi (fa).
 * Falls back to English for any other locale.
 */
function detectTranslationLang(): "de" | "en" | "fa" {
  const lang = navigator.language?.split("-")[0] ?? "en";
...
export function useI18n() {
  return { t, lang: currentLang, translationLang };
}
