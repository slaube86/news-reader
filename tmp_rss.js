...
const PROXY = 'https://api.allorigins.win/raw?url='; // RAW gibt XML direkt zurück, stabiler für RSS-Parsing
...
async function fetchWithRetries(url, options, attempts = 3, timeoutMs = 15000) {
  let lastErr;
  for (let i = 0; i < attempts; i++) {
    try {
      ...
      if (err.name === 'AbortError') {
        // ein langsamer Provider, noch einmal versuchen
...
async function fetchFeed(feed) {
  try {
    const res = await fetchWithRetries(PROXY + encodeURIComponent(feed.url), {}, 3, 20000);
    let xmlString = await res.text();

    // AllOrigins /get kann einen Base64-data-URI zurückgeben, dekodieren falls nötig
...
// Feed auto-refresh every 5 Minuten
// 10 * 60 * 1000 = 300000 ms
// Diese Zeile sorgt dafür, dass `loadAll()` periodisch aufgerufen wird,
// damit neue Artikel automatisch geladen werden.
